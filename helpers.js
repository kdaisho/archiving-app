...
const deleteFile = (appId, file) => {
	...
	if (fs.existsSync(filePath + file)) {
		fs.unlink(path.join(__dirname, `./dist/images/uploads/${appId}/${file}`), (error) => {
...
const writeItem = (dataString, appId, ts, fn) => {
	fs.writeFileSync(path.join(__dirname, `./data/applications/${appId}.json`), dataString);
	return fn(appId, ts);
};

const setLastUpdated = (appId, ts) => {
	return new Promise((resolve, reject) => {
		fs.readFile(path.join(__dirname, `./data/applications.json`), (error, data) => {
			if (error) {
				throw error;
			} else {
