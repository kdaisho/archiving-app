...
const createApplicationDirectory = (enteredName) => {
	...
	const applicationPath = path.join(__dirname, `data/applications/${appName}.json`);
...
const createCategoryName = (enteredName, isCategory) => {
	...
	if (name === "") {
		console.log(
			`Setup done with default ${isCategory ? "category and subcategory" : "subcategory"} name!`
				.bgBrightGreen.black
		);
		process.exit();
	}
	...
	for (let key in jsonData) {
		if (key === appName) {
			isCategory ? (jsonData[key].category = name) : (jsonData[key].subcategory = name);
			break;
		}
	}
	fs.writeFileSync(dataFilePath, JSON.stringify(jsonData, null, 4));
